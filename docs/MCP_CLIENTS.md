...
- **Local stdio with Bun:** `bunx --bun github:ahmedvnabil/Open-Emirates-Intelligence-MCP#main`
...
> `1.71.0` or newer. The GitHub command below runs the CI-tested `main` branch,
...
| Claude Code | `claude mcp add --transport http uae-intelligence https://uaemcp.zad.tools/mcp` | `claude mcp add uae-intelligence -- bunx --bun github:ahmedvnabil/Open-Emirates-Intelligence-MCP#main` | CLI; verify with `/mcp` |
...
| `bunx` is not found | Install Bun 1.3+, then run `bun --version` and `bunx --bun github:ahmedvnabil/Open-Emirates-Intelligence-MCP#main doctor` in a terminal. GUI apps may need an absolute path to `bunx` when their `PATH` differs from the shell. |
