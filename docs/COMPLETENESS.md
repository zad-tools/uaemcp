...
| MCP compliance | 9/10 | stdio and Streamable HTTP, 26 tools, resources and prompts | Compatibility must be retested on each SDK upgrade |
...
| Testing | 9/10 | 175 offline/runtime tests plus SDK and browser verification | Live authorities are intentionally not a deterministic CI gate |
...
| Preserve the original tool contract | Complete | 26 additive tools and compatibility assertions in `test/http.test.ts` |
