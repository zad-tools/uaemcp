...
| MCP compliance | 9/10 | stdio and Streamable HTTP, 22 tools, resources and prompts | Compatibility must be retested on each SDK upgrade |
...
| Preserve the original tool contract | Complete | 22 additive tools and compatibility assertions in `test/http.test.ts` |
