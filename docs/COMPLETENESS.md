...
| MCP compliance | 9/10 | stdio and Streamable HTTP, 40 tools, fourteen static resources, two resource templates and three prompts | Compatibility must be retested on each SDK upgrade |
...
| Preserve the original tool contract | Complete | 39 additive tools and compatibility assertions in `test/http.test.ts` |
...
- National coverage cannot be manufactured: the registry currently separates 45
  indexed official sources from thirteen live record connectors. Key-gated,
