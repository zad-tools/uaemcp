...
| MCP compliance | 9/10 | stdio and Streamable HTTP, 35 tools, nine static resources, two resource templates and three prompts | Compatibility must be retested on each SDK upgrade |
...
| Testing | 9/10 | 288 offline/runtime tests plus SDK and browser verification | Live authorities are intentionally not a deterministic CI gate |
...
| Preserve the original tool contract | Complete | 35 additive tools and compatibility assertions in `test/http.test.ts` |
...
- National coverage cannot be manufactured: the registry currently separates 41
  indexed official sources from nine live record connectors. Key-gated,
