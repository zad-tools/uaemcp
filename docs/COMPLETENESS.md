...
| MCP compliance | 9/10 | stdio and Streamable HTTP, 46 tools, seventeen static resources, two resource templates and three prompts | Compatibility must be retested on each SDK upgrade |
...
| Preserve the original tool contract | Complete | 46 tools including three agent-first facades and compatibility assertions in `test/http.test.ts` |
...
- National coverage cannot be manufactured: the registry currently separates 48
  indexed official sources from sixteen live record connectors. Key-gated,
