...
| MCP compliance | 9/10 | stdio and Streamable HTTP, 42 tools, sixteen static resources, two resource templates and three prompts | Compatibility must be retested on each SDK upgrade |
...
- National coverage cannot be manufactured: the registry currently separates 47
  indexed official sources from fifteen live record connectors. Key-gated,
