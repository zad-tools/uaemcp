...
| MCP compliance | 9/10 | stdio and Streamable HTTP, 39 tools, thirteen static resources, two resource templates and three prompts | Compatibility must be retested on each SDK upgrade |
...
- National coverage cannot be manufactured: the registry currently separates 44
  indexed official sources from twelve live record connectors. Key-gated,
