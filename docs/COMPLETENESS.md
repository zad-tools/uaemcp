...
| MCP compliance | 9/10 | stdio and Streamable HTTP, 43 tools, seventeen static resources, two resource templates and three prompts | Compatibility must be retested on each SDK upgrade |
...
| Preserve the original tool contract | Complete | 40 additive tools and compatibility assertions in `test/http.test.ts` |
