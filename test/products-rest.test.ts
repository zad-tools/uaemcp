...
describe("product registry REST contract", () => {
  it("exposes the same seven public products used by the website", async () => {
    const response = await handleRest(new Request("http://localhost/api/v1/products"));
    const payload = await response?.json();
    expect(response?.status).toBe(200);
    expect(payload.meta).toEqual({ total: 7, published: 7 });
    expect(payload.data[0]).toMatchObject({ id: "health_indicators", webPath: "/health-indicators", apiPath: "/api/v1/health-indicators" });
