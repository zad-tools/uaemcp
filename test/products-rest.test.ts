...
describe("product registry REST contract", () => {
  it("exposes the same seventeen public products used by the website", async () => {
    const response = await handleRest(new Request("http://localhost/api/v1/products"));
    const payload = await response?.json();
    expect(response?.status).toBe(200);
    expect(payload.meta).toEqual({ total: 17, published: 17 });
    expect(payload.data[0]).toMatchObject({ id: "evidence_studio", webPath: "/evidence-studio", apiPath: "/api/v1/evidence-dossier" });
