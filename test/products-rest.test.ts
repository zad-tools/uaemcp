...
describe("product registry REST contract", () => {
  it("exposes the same twenty-one public products used by the website", async () => {
    const response = await handleRest(new Request("http://localhost/api/v1/products"));
    const payload = await response?.json();
    expect(response?.status).toBe(200);
    expect(payload.meta).toEqual({ total: 21, published: 21 });
    expect(payload.data[0]).toMatchObject({ id: "connectivity_pulse", webPath: "/connectivity", apiPath: "/api/v1/connectivity" });
